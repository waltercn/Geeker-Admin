...
export const getCaptchaApi = () => {
  return http.post<Login.CaptchaImage>(PORT3 + `/captcha/load`, {}, { loading: false });
};
...
export const loginApi = (params: Login.ReqLoginForm) => {
  return http.post<Login.ResLogin>(PORT3 + `/auth/login`, params, { loading: false }); // 正常 post json 请求  ==>  application/json
...
export const getAuthButtonListApi = () => {
  return http.get<Login.ResAuthButtons>(PORT4 + `/auth/buttons`, {}, { loading: false });
...
export const logoutApi = () => {
  ...
  return http.post(PORT3 + `/auth/logout`);
};
